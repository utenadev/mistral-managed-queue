...
Un serveur MCP (Model Context Protocol) et un outil CLI qui coordonne les appels locaux et multi-processus/multi-clients à l'offre gratuite de Mistral (~1 requête / 30 secondes) via une file d'attente SQLite partagée.
...
**Package :** [`mcp-mistral-queue`](https://pypi.org/project/mcp-mistral-queue/) sur PyPI · **script console :** `mmq` (pas le nom du package) · **version actuelle :** `0.1.2`
 ...
 * **Contrôle multi-processus et de priorité :** Plusieurs processus/tâches peuvent mettre des travaux en file d'attente. La priorité (1–3) ainsi que l'ordre de traitement de la file (un seul vol) organisent la file.
 * **Options flexibles de modèle et de message :** N'importe quel nom de modèle de chat Mistral (par défaut `mistral-small-latest` ; ex. `mistral-large-latest`, `codestral-latest`), ainsi que l'historique complet de la conversation via un tableau `messages`.
 ...
 * **Mistral Vibe / Grok / Claude Desktop :** Enregistrement en tant que serveur MCP (`mmq --mcp`). **Ne pas** utiliser `vibe mmq.py "..."` — cela lance l'agent CLI de Vibe, et non cet outil.
 * **Adapté à l'offre gratuite :** Tâches occasionnelles (ex. traduction de documentation) qui peuvent attendre ~31 secondes entre les appels sans épuiser une pile dédiée de limites de débit.
 * **CLI convivial pour l'IA :** Conçu pour les agents de codage (Vibe, Claude Code, etc.) avec les sous-commandes `docs list` / `docs show`, des conseils pour les agents dans les textes d'aide et des sorties JSON pour un traitement facile.

## Prérequis

 * Python 3.10+
 * [uv](https://github.com/astral-sh/uv) recommandé (`uvx` / `uv run`) ; `pip` fonctionne aussi
 ...
 * Le nom du script console est **`mmq`**. Incorrect : `uvx mcp-mistral-queue --mcp`. Correct : `uvx --from mcp-mistral-queue mmq --mcp`.
 * Dépendances : `mcp[cli]>=1.0.0,<2`, `mistralai>=1.0.0,<2` (incluses par le package).

## Utilisation

### 1. Mode CLI

Après installation via PyPI ou `uvx`, invoquez **`mmq`**.
Depuis un dépôt git, vous pouvez toujours utiliser `uv run mmq.py ...` (PEP 723).
...
### Commandes de documentation conviviales pour l'IA

Pour les agents de codage (Vibe, Claude Code, etc.) :
...
La commande `docs list` produit une sortie JSON avec des descriptions pour un traitement facile :
...
Après avoir modifié la configuration, redémarrez le client. Liste de vérification manuelle pour Vibe : [docs/SMOKE_VIBE.md](docs/SMOKE_VIBE.md).
...
| `MMQ_TEMP_DB_PATH` | par utilisateur dans un répertoire temporaire | Chemin du fichier de la base de données de la file d'attente partagée |
...
| `MMQ_FAKE_API` | désactivé | Hors ligne/test unitaire : client factice (`1`/`true`) |
...
|---|---|---|---|
| prompt | chaîne de caractères | null | Texte de l'invite utilisateur ponctuel |
...
| system_prompt | chaîne de caractères | null | Invite système personnalisée (uniquement avec `prompt`) |
| priority | nombre | 2 | Priorité de la tâche (1 : haute, 2 : normale, 3 : basse) |

#### `get_queue_status`

Retourne l'état actuel de la file d'attente partagée et des limites de débit au format JSON :

| Champ | Type | Description |
|---|---|---|
| pending | nombre | Tâches en attente dans la file |
| processing | nombre | Tâches actuellement revendiquées/exécutées |
...
| in_flight | booléen | Si une tâche est en cours de traitement |
...
Les tests e2e utilisent `MMQ_FAKE_API=1` et une courte `MMQ_BASE_WAIT_TIME` pour exercer les limites de processus (CLI / stdio de MCP).
...
| Peut attendre ~31 secondes | ja puis fr prennent chacun un créneau réservé |
...
1. Protège les blocs de code délimités (FSM de ligne) et les `code` en ligne avec des espaces réservés
2. Met en file d'attente une tâche de traduction par langue via **`execute_mistral_queue_async`**
3. Restaure les espaces réservés, corrige le sélecteur de langue, valide (ex. équilibrage des délimitations)
...
- **@fujibee** pour les informations sur l'utilisation des files d'attente avec le mode WAL de SQLite (#agmsg).
- **shunsuke_suzuki** pour la méthodologie de développement de CLI conviviale pour l'IA
 ...
 * [docs/SMOKE_VIBE.md](docs/SMOKE_VIBE.md) — test manuel Vibe / MCP
 * [docs/SEARCH_POSITIONING.md](docs/SEARCH_POSITIONING.md) — où la recherche web doit être placée (hors base mmq)
