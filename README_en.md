...
**Vibe config example** (see [docs/SMOKE_VIBE.md](docs/SMOKE_VIBE.md) for details):
...
After changing config, restart Vibe and have the agent use the `ask_mistral` tool (pass `model` when needed, e.g. `mistral-large-latest`).
...
### MCP tool: `ask_mistral`

When the server is running, clients can call `ask_mistral`:
 ...
 * Queue DB: `/tmp/mcp_mistral_queue_<USER>/mcp_mistral_flow_control.db`
